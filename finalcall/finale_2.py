...
ATTENTION_ID,PHOTO,ATT_VALUES = range(3)


# Conversation states for /schema command
SCHE_NAME, SCHE_DESP,FIELD_NAME,CONFIRM_FIELD_NAME,SCHEMA_ID = range(5)
...
async def recieve_schema_descp(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text('Enter all the Field Names here with comma seperated:')
    return FIELD_NAME
...
async def recieve_field_name(update: Update, context: CallbackContext):
    ...
    global ip_data
    field_names = update.message.text.split(',')

    for name in field_names:
        ip_data["data"].append({"name": name.strip(), "type": "string"})
...
# Ask for Schema ID
    await update.message.reply_text('Enter Schema ID:')
    return SCHEMA_ID

async def receive_schema_id(update: Update, context: CallbackContext):
    global schema_id1
    schema_id1 = update.message.text

    # Optionally, you can perform validation or additional processing here

    await update.message.reply_text(f'Schema ID stored: {schema_id1}')

    await update.message.reply_text('Enter the values to be attested corresponding to the field name (separated by commas):')
    return ATT_VALUES


async def receive_attestation_values(update: Update, context: CallbackContext):
    global att_values
    att_values = update.message.text.split(',')

    # Optionally, you can perform validation or additional processing here

    await update.message.reply_text(f'Values to be attested stored: {att_values}')
    ...
    def final_attest():
        ...
        try:
            print(f"{update.message.from_user.id}")
            data = supabase.table("wallets").select("pub_key,priv_key").eq("tel_id", update.message.from_user.id).execute()
            ...
            try:
                ethsign = web3.eth.contract(
                    address=Web3.to_checksum_address("0x4e4af2a21ebf62850fd99eb6253e1efbb56098cd"), abi=abi)
                nonce = web3.eth.get_transaction_count(address)
                int_schema_id = int(schema_id1, 16)
                inp_data = encode(["string", "string",],att_values )
                attest_tx = ethsign.functions.attest(
                    [int_schema_id, 0, 0, 0, address, 0, 0, False, [attest_id], inp_data], 'Ghili', '0x',
                    '0x').build_transaction(
                    {"chainId": 421614, "gas": 1650000, "maxFeePerGas": web3.to_wei("2", "gwei"),
                     "maxPriorityFeePerGas": web3.to_wei("1", "gwei"), "nonce": nonce, })
                print(attest_tx)
                signed_txn = web3.eth.account.sign_transaction(attest_tx, private_key=priv_key)
                print(signed_txn)
                web3.eth.send_raw_transaction(signed_txn.rawTransaction)
                print(f"Transaction Sent Successsfully: https://sepolia.arbiscan.io/tx/{signed_txn.hash.hex()}")
                tx_receipt = web3.eth.get_transaction_receipt(signed_txn.hash.hex())
...
async def viewall_command(update: Update, context: CallbackContext):  # i have assigned each variable to each parameters like attestaion id and more.
    ...
    await update.message.reply_text(f'From Address: {add1}')
...
async def generate_certificate(attest_id,from_address_id,to_address_id, schema_name,schema_desp, chat_id, bot):
    ...
    c.drawString(227, 540, f"attid{add1}")


    c.setFont("Helvetica", 18)
    c.setFillColorRGB(1, 1, 1)
    c.drawString(227, 475, f"time{start_datetime}")

    # Add attestation and schema name to the certificate
    c.setFont("Helvetica", 18)
    c.setFillColorRGB(1, 1, 1)
    c.drawString(227, 400, f'to{attest_id}')

    # Add attestation and schema name to the certificate
    c.setFont("Helvetica", 18)
    c.setFillColorRGB(1, 1, 1)
    c.drawString(227, 330, f'sname{schema_name}')


    c.setFont("Helvetica", 18)
    c.setFillColorRGB(1, 1, 1)
    c.drawString(227, 200, f"txhas{tx}")

    c.setFont("Helvetica", 18)
    c.setFillColorRGB(1, 1, 1)
    c.drawString(227, 85, f"sdesp{schema_desp}")
...
if __name__ == '__main__':
    ...
    attest_handler = ConversationHandler(
        ...
        states={
            ...
            ATTENTION_ID: [MessageHandler(filters.TEXT, receive_attestation_id)],
            SCHEMA_ID: [MessageHandler(filters.TEXT, receive_schema_id)],
            ATT_VALUES: [MessageHandler(filters.TEXT, receive_attestation_values)],
        },
