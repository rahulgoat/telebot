...
SCHE_NAME, SCHE_DESP,FIELD_NAME,CONFIRM_FIELD_NAME = range(4)
...
async def receive_attestation_id(update: Update, context: CallbackContext):
    ...
    # You might want to do some validation on attest_id here

    # Prompt for the photo
    await update.message.reply_text('Enter the photo to be attested:')
    # Return the state
    return PHOTO
...
async def receive_photo(update: Update, context: CallbackContext):
    ...
    attest_photo = photo_file.download('attest.png')

    # You can perform further processing if needed

    # Inform the user about successful submission
    await update.message.reply_text('You have successfully entered all your details.')
